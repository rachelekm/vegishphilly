...
function Sidebar({ restaurants, mapData }: SidebarProps) {
  ...
  useEffect(() => {
    mapData.data.bounds &&
      currentPag &&
      store.dispatch(restaurantsFetch([mapData.data.bounds, currentPag]));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPag]);

  const renderedRestaurants =
    allRestaurants && allRestaurants.features.length > 0
      ? allRestaurants.features.map((feature) => {
          return (
            <li className="restaurant-item" key={allRestaurants.features.indexOf(feature)}>
              ...
              {feature.properties.average_rating ? (
                <h4 className="item-overall-rating">
                  Avg Rating: {feature.properties.average_rating} stars
                </h4>
              ) : (
  ...
  return (
    <Flex className="sidebar">
      <div className="search-box">Search city here</div>
      <ul className="sidebar-list">{renderedRestaurants}</ul>
      {pag_count > 1 && (
          ...
          <ul className="pagination">
            {prev ? (
              prev.includes("page=") ? (
                <li key="prev">
                  <button
                    onClick={(event: React.MouseEvent<HTMLElement>) =>
                      setCurentPag(parseInt(prev?.slice(-1)))
                    }
                  >
                    prev
                  </button>
                ...
                <li key="prev">
                  <button onClick={(event: React.MouseEvent<HTMLElement>) => setCurentPag(1)}>
                    prev
                  </button>
                </li>
              )
            ...
            {[...Array(pag_count)].map((v: undefined, i: number) => {
              return (
                <li key={i + 1}>
                  <button onClick={(event: React.MouseEvent<HTMLElement>) => setCurentPag(i + 1)}>
                    {i + 1}
                  </button>
                </li>
              );
            ...
            {next ? (
              <li key="next">
                <button
                  onClick={(event: React.MouseEvent<HTMLElement>) =>
                    setCurentPag(parseInt(next?.slice(-1)))
                  }
                >
                  next
                </button>
