...
import { LoginCredentials, PaginatedRestaurants, PathTransition, User } from "./models";
...
export async function fetchRestaurants(
  ...
  return new Promise((resolve, reject) => {
    ...
    api
      ...
      .then((response) =>
        decodeResponse(paginatedRestaurantsDecoder, response.data).then(resolve).catch(reject)
      )
      .catch((error) => {
        return error.response
          ? decodeResponse(apiErrorDecoder, error.response.data)
